...
import { IDayStatistic } from './StatisticsPage';
  ...
  devtools(
    ...
    persist((set) => ({
      ...
      incFullTime: () =>
        set((state) => ({
          statistics: state.statistics.map((item, index) =>
            index === (state.statistics.length - 1) ? { ...item, fullTime: item.fullTime += 1 } : item)
        })),
      // увеличение количества остановок
      incStopsNumb: () =>
        set((state) => ({
          statistics: state.statistics.map((item, index) =>
            index === (state.statistics.length - 1) ? { ...item, stopsNumb: item.stopsNumb += 1 } : item)
        })),
      // увеличение времени непосредственного выполнения задачи
      incWorkTime: () =>
        set((state) => ({
          statistics: state.statistics.map((item, index) =>
            index === (state.statistics.length - 1) ? { ...item, workTime: item.workTime += 1 } : item)
        })),
      // увеличение времени на паузе
      incPauseTime: () =>
        set((state) => ({
          statistics: state.statistics.map((item, index) =>
            index === (state.statistics.length - 1) ? { ...item, pauseTime: item.pauseTime += 1 } : item)
        })),
      ...
      increasePomodoroTotal: (task) =>
        set((state) => ({
          tasks: state.tasks.map((item) =>
            item.id === task.id ? { ...item, pomodoroTotal: (task.pomodoroTotal += 1) } : item
          ),
        })),
      ...
      increasePomodoroDone: (task) =>
        set((state) => ({
          tasks: state.tasks.map((item) =>
            item.id === task.id ? { ...item, pomodoroDone: (task.pomodoroDone += 1) } : item
          ),
        })),
      ...
      incPomDoneToday: () =>
        set((state) => ({
          statistics: state.statistics.map((item, index) =>
            index === (state.statistics.length - 1) ? { ...item, pomDoneToday: item.pomDoneToday += 1 } : item)
        })),
